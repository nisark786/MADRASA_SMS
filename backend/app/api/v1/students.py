...
async def list_students(
    ...
    """List all students. Cached in Redis, invalidated on any write."""
    # ⚡ Cache hit path (~5ms)
    cached = await rc.get_cached_response(_STUDENTS_LIST_KEY)
    if cached is not None:
        return cached

    # ❄️ Cache miss: query DB
    result = await db.execute(
        select(*_STUDENT_COLS).order_by(Student.created_at.desc())
    )
    rows = [dict(r) for r in result.mappings().all()]
    await rc.cache_response(_STUDENTS_LIST_KEY, rows)
    return rows
...
async def create_student(
    ...
    existing = await db.execute(
        select(Student.id).where(Student.email == body.email).limit(1)
    )
...
async def update_student(
    ...
    """Update student details. Invalidates list cache."""
    ...
    if body.email is not None:
        existing = await db.execute(
            select(Student.id).where(Student.email == body.email, Student.id != student_id).limit(1)
        )
        if existing.scalar_one_or_none():
            raise HTTPException(status_code=400, detail="Email already in use")
        updates["email"] = body.email
    if body.enrollment_date is not None:
        updates["enrollment_date"] = datetime.fromisoformat(body.enrollment_date)
...
async def submit_student_form(body: CreateStudentRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Invalidate list cache since a new public student was added
    await rc.invalidate_keys(_STUDENTS_LIST_KEY)
